...
fn solve2x3(i_matrix: [2][3]i32) error{NoIntegerSolutions}![2]i32 {
    const V = @Vector(3, i32);
    var matrix: [2]V = undefined;
    for (&i_matrix, 0..) |row, i| matrix[i] = row;
    // clear the first column
    while (matrix[0][0] != 0 and matrix[1][0] != 0) {
        const a: i32 = @intCast(@abs(matrix[0][0]));
        const b: i32 = @intCast(@abs(matrix[1][0]));
...
fn process(input: []const u8) !i32 {
    ...
    while (machine_iter.next()) |machine| {
        ...
        const a_presses, const b_presses = solve2x3(.{ .{ a_x, b_x, p_x }, .{ a_y, b_y, p_y } }) catch continue;
        tokens_count += 3 * a_presses + b_presses;
    }
    return tokens_count;
}

fn parse(line: []const u8) !struct { i32, i32 } {
    if (std.mem.startsWith(u8, line, "Button ")) {
        ...
        const x = try std.fmt.parseInt(i32, line[plus..comma], 10);
        const plus_again = std.mem.indexOfScalarPos(u8, line, comma, '+') orelse return error.ParseFailed;
        const y = try std.fmt.parseInt(i32, line[plus_again + 1 ..], 10);
        return .{ x, y };
    }
    ...
    const x = try std.fmt.parseInt(i32, line[equals + 1 .. comma], 10);
    const equals_again = std.mem.indexOfScalarPos(u8, line, comma, '=') orelse return error.ParseFailed;
    const y = try std.fmt.parseInt(i32, line[equals_again + 1 ..], 10);
    return .{ x, y };
}
