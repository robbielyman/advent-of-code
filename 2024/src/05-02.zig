...
fn process(gpa: std.mem.Allocator, input: []const u8) !u32 {
    ...
    while (iter.next()) |line| {
        ...
        while (tokenizer.next()) |token| {
            ...
            if (std.mem.indexOfAny(u32, rule, list.items)) |_| correct = false;
            try list.append(allocator, new);
        }
        if (correct) continue;
        std.mem.sort(u32, list.items, rules, lessThan);
...
/// a is less than b if a|b is a rule and b|a is not a rule (or if neither is present)
fn lessThan(rules: std.AutoHashMapUnmanaged(u32, []u32), a: u32, b: u32) bool {
    const @"a|b" = rule: {
        const rule = rules.get(a) orelse break :rule true;
        break :rule if (std.mem.indexOfScalar(u32, rule, b)) |_| true else false;
    };
    const @"b|a" = rule: {
        const rule = rules.get(b) orelse break :rule false;
        break :rule if (std.mem.indexOfScalar(u32, rule, a)) |_| true else false;
    };
    return @"a|b" and !@"b|a";
}
...
test {
    ...
    try std.testing.expectEqual(123, output);
}
