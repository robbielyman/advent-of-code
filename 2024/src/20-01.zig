...
fn process(allocator: std.mem.Allocator, input: []const u8, cheats_at_least: u32) !u32 {
    ...
    for (0..track.len - 2) |i| {
        ...
        for (i + 2..track.len) |j| {
            ...
            if (cheat_start.dist(cheat_end) != 2) continue;
            const cheat_amount = j - i - 2;
            if (cheat_amount >= cheats_at_least) count += 1;
        }
    }
    return count;
}
...
test {
    ...
    const conditions: []const u32 = &.{ 20, 38, 40, 64 };
    for (&output, conditions) |*out, condition|
        out.* = try process(std.testing.allocator, input, condition);
    try std.testing.expectEqualSlices(u32, &.{ 5, 3, 2, 1 }, &output);
}
