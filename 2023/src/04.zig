...
fn processLine(line: []const u8) !u64 {
    const red, const green, const blue = blk: {
        const remainder = remainder: {
            var iterator = std.mem.tokenizeSequence(u8, line, ": ");
            _ = iterator.next() orelse return error.ParseFailed;
            const remainder = iterator.rest();
            break :remainder remainder;
        };
        ...
        break :blk .{ colors[0], colors[1], colors[2]};
    };
...
test "processLine" {
    ...
    const expectations: [] const u64 = &.{ 48, 12, 1560, 630, 36 };
