...
fn walk(walkers: *Walker, paths: *Paths, map: []const []const u8) !usize {
    while (walkers.removeOrNull()) |current| {
        ...
        if (current.y + 1 == map.len and current.x + 1 == map[map.len - 1].len) {
            return cost;
        }
        const directions: [4]?Data = .{
            if (current.x > 0 and current.dir != .right and (current.dir != .left or current.rem > 0))
                .{
                    .x = current.x - 1,
                    .y = current.y,
                    .dir = .left,
                    .rem = if (current.dir != .left) 2 else current.rem - 1,
                }
            else
                null,
            if (current.x + 1 < map[current.y].len and current.dir != .left and (current.dir != .right or current.rem > 0))
                .{
                    .x = current.x + 1,
                    .y = current.y,
                    .dir = .right,
                    .rem = if (current.dir != .right) 2 else current.rem - 1,
                }
            else
                null,
            if (current.y > 0 and current.dir != .down and (current.dir != .up or current.rem > 0))
                .{
                    .x = current.x,
                    .y = current.y - 1,
                    .dir = .up,
                    .rem = if (current.dir != .up) 2 else current.rem - 1,
                }
            else
                null,
            if (current.y + 1 < map.len and current.dir != .up and (current.dir != .down or current.rem > 0))
                .{
                    .x = current.x,
                    .y = current.y + 1,
                    .dir = .down,
                    .rem = if (current.dir != .down) 2 else current.rem - 1,
                }
            else
                null,
        };
...
test "walk map" {
    ...
    const count = try walk(&walkers, &paths, map);
    try std.testing.expectEqual(@as(usize, 102), count);
}
