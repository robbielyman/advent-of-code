...
pub fn main() !void {
    ...
    const count: i64 = @divExact(@abs(area) + @as(i64,perimeter), 2) + 1;
...
test "parseInput" {
    ...
    try std.testing.expectEqual(@as(i32, 62), count);
}

fn shoelace(line: []const u8, x: *i32, y: *i32, area: *i32, perimeter: *i32) !void {
    const new_x, const new_y = coords: {
        const idx = std.mem.indexOfPos(u8, line, 2, " ") orelse return error.ParseFailed;
        const number = try std.fmt.parseUnsigned(i32, line[2..idx], 10);
        perimeter.* += number;
        switch (line[0]) {
            'R' => break :coords .{ x.* + number, y.* },
            'L' => break :coords .{ x.* - number, y.* },
            'U' => break :coords .{ x.*, y.* + number },
            'D' => break :coords .{ x.*, y.* - number },
            else => unreachable,
        }
    };
