...
pub fn main() !void {
    ...
    for (gals, 1..) |a, i| {
        if (i == gals.len) break;
        for (gals[i..]) |b| {
            const coords = expandedCoordinates(rows, cols, a, b, 1000000);
...
fn expandedCoordinates(
    ...
    const row_offset = row_offset: {
        ...
        break :row_offset if (b_offset.? > a_offset.?)
            (b_offset.? - a_offset.?) * (expansion_constant - 1)
        else
            (a_offset.? - b_offset.?) * (expansion_constant - 1);
    };
...
test "manhattanDistance and expandedCoordinates" {
    ...
    const new_coords = expandedCoordinates(rows, cols, galaxies[0], galaxies[1], 2);
    try std.testing.expectEqual(expected, new_coords);

    const expanded: []const [2][2]usize = &.{
        expandedCoordinates(rows, cols, galaxies[0], galaxies[6], 2),
        expandedCoordinates(rows, cols, galaxies[2], galaxies[5], 2),
        expandedCoordinates(rows, cols, galaxies[7], galaxies[8], 2),
    };
    ...
    for (galaxies, 1..) |a, i| {
        ...
        for (galaxies[i..]) |b| {
            const expanded_coords = expandedCoordinates(rows, cols, a, b, 10);
            count += manhattanDistance(expanded_coords[0], expanded_coords[1]);
        }
    }
    try std.testing.expectEqual(@as(usize, 1030), count);
}
