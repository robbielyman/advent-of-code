...
pub fn process(allocator: std.mem.Allocator, input: []const u8) !u32 {
    ...
    return try weigh(&nodes, allocator);
}

fn weigh(nodes: *Nodes, allocator: std.mem.Allocator) !u32 {
    for (nodes.values()) |*val| {
        if (val.list.items.len != 0) continue;
        val.weighed = val.weight;
    }
    var list: std.ArrayListUnmanaged(Children.Weight) = .empty;
    defer list.deinit(allocator);
    while (true) {
        outer: for (nodes.values()) |*val| {
            list.clearRetainingCapacity();
            if (val.weighed) |_| continue;
            for (val.list.items) |child_key| {
                const child = nodes.get(child_key).?;
                const weight = child.weighed orelse continue :outer;
                try list.append(allocator, weight);
            }
            if (indexOfDifferent(Children.Weight, list.items)) |ret| {
                const different, const others = ret;
                const weight = nodes.get(val.list.items[different]).?.weight;
                return others.num() + weight.num() - list.items[different].num();
            }
            var sum = val.weight.num();
            for (list.items) |item| sum += item.num();
            val.weighed = .init(sum);
        }
    }
}

fn indexOfDifferent(comptime T: type, slice: []const T) ?struct { usize, T } {
    if (slice.len == 0) return null;
    if (std.mem.allEqual(T, slice, slice[0])) return null;
    if (slice.len == 2) return .{ 1, slice[0] };
    // is the first one the odd one out?
    if (std.mem.allEqual(T, slice[1..], slice[1])) return .{ 0, slice[1] };
    return .{ std.mem.indexOfNone(T, slice, &.{slice[0]}).?, slice[0] };
}
