...
fn process(input: &str) -> i32 {
    let (horiz, vert, _aim) = input.lines().fold((0, 0, 0), |(horiz, vert, aim), line| {
        let (direction, remainder) = line.split_once(' ').unwrap();
        let change: i32 = remainder.parse().unwrap();
        match direction {
            "forward" => (horiz + change, vert + (aim * change), aim),
            "up" => (horiz, vert, aim - change),
            "down" => (horiz, vert, aim + change),
            _ => panic!("bad input!"),
        }
    });
    horiz * vert
